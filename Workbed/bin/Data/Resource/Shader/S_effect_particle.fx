...
void GS (point PS_INPUT input[1], inout TriangleStream<PS_INPUT> triStream)
{
	...
	{
		float3 rotation = 0;
		rotation.x = g_positions[i].x * cos(input[0].Rotation * input[0].Time) - sin (input[0].Rotation * input[0].Time) * g_positions[i].y;
		rotation.y = sin(input[0].Rotation * input[0].Time) * g_positions[i].x + cos(input[0].Rotation* input[0].Time) * g_positions[i].y;

		output.Pos = float4(float4(input[0].Size * rotation, 0) + input[0].Pos);
		//output.Pos = (g_positions[i] * input[0].Size) + input[0].Pos;
